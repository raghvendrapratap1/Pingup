...
const UserCard = ({ user, onUserUpdate }) => {
    ...
    useEffect(() => {
        console.log(`🔄 UserCard ${user?.full_name}: isFollowing = ${isFollowing}, connectionStatus = ${connectionStatus}`)
    }, [isFollowing, connectionStatus, user?.full_name])

    // Debug: Log component render
    console.log(`🎨 Rendering UserCard for ${user?.full_name}: isFollowing=${isFollowing}, connectionStatus=${connectionStatus}`)
    ...
    const handleConnectionRequest = async () => {
        console.log('🔗 Connect button clicked! Current status:', connectionStatus)
        
        // Handle different states with appropriate toasts
        if (connectionStatus === 'accepted') {
            console.log('📱 Showing connected toast')
        ...
        try {
            setLoading(true)
            console.log(`🔗 Sending connection request to ${user?.full_name}...`)
            console.log('🔗 User ID being sent:', user._id)
            console.log('🔗 Request payload:', { id: user._id })
            const { data } = await api.post('/api/user/connect', { id: user._id })
            console.log('🔗 Connection request response:', data)
            if (data.success) {
                setConnectionStatus('pending')
                console.log(`✅ Connection request sent successfully to ${user?.full_name}`)
                ...
                console.log(`❌ Connection request failed: ${data.message}`)
